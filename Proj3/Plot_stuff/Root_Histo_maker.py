...
class ROOT_Histo_Maker:
    ...
    *---------------------------------------------------------------------------------------------------*
    ...
    def __init__(self,
        ...
        self.fig, (self.ax1, self.ax2) = plt.subplots(2, 1, gridspec_kw={'height_ratios': [3, 1]}, num=0, dpi=80, facecolor='w', edgecolor='k',figsize = (7.4, 5.8))
    ...
    def createHisto(self):
        N, bins = np.histogram(self.Data, bins = self.nr_bins, range=(self.bin_min, self.bin_max))
        ...
        n, bins, patches = self.ax1.hist(self.MC_Data, 
    ...
    def calcN(self, bins, weight, values):
        ...
        for j in range(len(bins)-1):
            ...
            for i in range(len(values)):
                s += np.sum(weight[i][(values[i] < bins[j+1]) * (values[i] > bins[j])])
            n.append(s)
        return n
