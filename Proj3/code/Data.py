...
class Dataset:
    def __init__(self, X, Y, DataKeys):
        self.features = X
        self.targets = Y
        self.column_names = DataKeys
    ...
    def featurePlot(self):
        ...
        plt.plot(np.arange(meanB.shape[0]), meanB/meanTot, label = "Background")
        plt.plot(np.arange(meanB.shape[0]), meanS/meanTot, "--", label = "Signal")
...
if __name__ == "__main__":
    ...
    DataKeys = data.keys()[1:-2]
    print(DataKeys)

    data_obj = Dataset(X, Y, DataKeys)
